"""Celery Scheduled Tasks - Background job execution for ETL, ML scoring, and analytics"""
...
from backend2.module_runner import (
    ensure_success,
    run_python_module,
    summarize_result,
)
...
def _run_etl_stage(module_name: str, stage_name: str) -> dict[str, Any]:
    """Execute a single ETL module stage"""
...
def system_health_task(self) -> dict[str, Any]:
    """System health check - Redis connectivity and basic status"""
    health = {
    ...
    logger.info(
        ...
        extra={"event": "system_health", "extra_data": health}
    )
    return health
...
@celery_app.task(
    bind=True,
    name="backend2.tasks.etl_full_refresh_task",
    autoretry_for=(Exception,),
    retry_backoff=True,
    retry_kwargs={"max_retries": 3},
)
def etl_full_refresh_task(self, source_mode: str = "marketai") -> dict[str, Any]:
    """Execute full ETL workflow: import/extract → transform → load → cache invalidate"""
    first = (
        etl_import_marketai_task.s()
        if source_mode == "marketai"
        else etl_extract_from_sql_dump_task.s()
    )
    ...
    workflow = chain(
        ...
        etl_load_task.s(),
        invalidate_cache_task.s(patterns=["company_profile:*", "financial_trend:*", "dashboard_summary:*"]),
    )
    ...
    logger.info(
        "ETL workflow started",
        extra={"event": "etl_workflow_started", "extra_data": payload}
    )
    return payload
...
def run_etl_pipeline_task(self) -> dict[str, Any]:
    """
    Daily 1:00 AM IST — Run ETL scripts 2 and 3 (clean, transform, and load to warehouse).
    """
    workflow = chain(
        etl_transform_task.s(),
        etl_load_task.s(),
    ...
    logger.info(
        "ETL pipeline task started",
        extra={"event": "etl_pipeline_started", "extra_data": payload}
    )
    return payload
...
def score_all_companies_task(self, previous_result: dict | None = None) -> dict[str, Any]:
    """
    Daily 2:00 AM IST — Recalculate health scores for all 100 companies.
    Runs ML scoring module to compute overall_score, profitability, growth, leverage,
    cashflow, dividend scores.
    """
    ...
    logger.info(
        "Company scoring completed",
        extra={"event": "companies_scored", "extra_data": payload}
    )
    return payload
...
def generate_pros_cons_task(self, previous_result: dict | None = None) -> dict[str, Any]:
    """
    Daily 2:30 AM IST — Re-run pros/cons rule engine for all companies.
    Uses fact_ml_scores and fact tables to generate pro/con statements based on
    financial metrics.
    """
    try:
        ...
        logger.info(
            ...
            extra={"event": "pros_cons_generated", "extra_data": payload}
        )
        return payload
    except Exception as e:
        logger.error(
            f"Pros/cons generation failed: {str(e)}",
            extra={"event": "pros_cons_failed"}
        )
        raise
...
def detect_anomalies_task(self, previous_result: dict | None = None) -> dict[str, Any]:
    """
    Weekly Sunday 3:00 AM IST — Run anomaly detection (Z-score + Isolation Forest).
    Detects financial anomalies and stores flags in fact_anomaly_flags.
    """
    try:
        from etl.anomaly_detector import (
            detect_anomalies_isolation_forest,
            detect_anomalies_zscore,
        )

        zscore_result = detect_anomalies_zscore()
        isolation_forest_result = detect_anomalies_isolation_forest()

        payload = {
            "stage_name": "detect_anomalies",
            "zscore_anomalies": zscore_result.get("count", 0),
            "isolation_forest_anomalies": isolation_forest_result.get("count", 0),
            "previous_result": previous_result,
            "timestamp": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        }
        logger.info(
            "Anomaly detection completed",
            extra={"event": "anomaly_detection_complete", "extra_data": payload}
        )
        return payload
    except Exception as e:
...
def detect_trends_task(self, previous_result: dict | None = None) -> dict[str, Any]:
    """
    Weekly Sunday 4:00 AM IST — Trend analysis & revenue forecasting.
    Linear regression for trend classification (UP/FLAT/DOWN).
    Holt-Winters forecasting for top 20 companies.
    """
    try:
        from etl.trend_analyzer import (
            analyze_all_trends,
            forecast_revenue,
        )

        trend_result = analyze_all_trends()
        forecast_result = forecast_revenue()

        payload = {
            "stage_name": "detect_trends",
            "companies_analyzed": trend_result.get("count", 0),
            "companies_forecasted": forecast_result.get("count", 0),
            "previous_result": previous_result,
            "timestamp": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        }
        logger.info(
            "Trend analysis completed",
            extra={"event": "trend_analysis_complete", "extra_data": payload}
        )
        return payload
    except Exception as e:
