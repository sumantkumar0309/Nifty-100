"""Pros and Cons Generator - Auto-generate pro/con statements based on financial metrics"""
...
def generate_all_pros_cons() -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Generated pros/cons for {result['count']} companies",
...
def _evaluate_pros_rules(row: pd.Series) -> list[str]:
    """Evaluate all pro rules for a company. Returns list of pro statements."""
    pros = []
    ...
    try:
        ...
        # Rule 2: 3Y ROE > 20% → "Company has good ROE track record"
        if pd.notna(row['growth_3y']) and row['growth_3y'] > 20:
            pros.append(f"Company has strong growth track record: 3Y CAGR {row['growth_3y']:.1f}%")
        
        # Rule 3: Dividend payout consistently > 30%
        if pd.notna(row['dividend_payout_pct']) and row['dividend_payout_pct'] > 30:
            pros.append(f"Consistent dividend payout of {row['dividend_payout_pct']:.1f}%")
        
        # Rule 4: Operating cash flow > net profit for quality
...
def _evaluate_cons_rules(row: pd.Series) -> list[str]:
    """Evaluate all con rules for a company. Returns list of con statements."""
    cons = []
    
    try:
        # Rule 1: D/E > 1.5 → "Stock carries high debt"
        ...
        # Rule 3: D/E > 2.0 (Very high leverage)
        ...
        # Rule 4: Low profitability score
