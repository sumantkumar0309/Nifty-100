"""
Channel Partner Authentication Module

Implements token-based authentication for API access by channel partners.
Includes:
...
def validate_api_token(token: str, secret: str) -> tuple[bool, Optional[str], Optional[dict[str, Any]]]:
    ...
    try:
        ...
        result = session.execute(query, {'token_hash': token_hash, 'secret_hash': secret_hash}).fetchone()
        session.close()
...
def log_api_usage(
    ...
    try:
        ...
        insert_query = text("""
            ...
            (partner_id, endpoint, method, response_status, response_time_ms, request_size_bytes, response_size_bytes, timestamp)
            VALUES 
            (:partner_id, :endpoint, :method, :response_status, :response_time_ms, :request_size_bytes, :response_size_bytes, :timestamp)
        """)
